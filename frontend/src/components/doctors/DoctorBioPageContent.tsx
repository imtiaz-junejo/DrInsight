...
const BADGE_COLORS = ["#7c3aed", "#dc2626", "#0891b2", "#9333ea", "#059669", "#d97706"];
...
function MiniDoctorRow({
  ...
  if (variant === "colleague") {
    return (
      <div className="colleague-card">
        ...
        <Link href={`/our-doctors/${doctor.id}`} className="follow-sm">
          View
        </Link>
      </div>
    );
  ...
  return (
    <Link href={`/our-doctors/${doctor.id}`} className="other-doc-item">
    ...
    <Link href={`/blog/${article.slug}`} className="art-card-bio">
      ...
      <div className="art-info">
        ...
        <div className="art-meta-bio">
          ...
          {formatViews(article.viewCount)} views · {article.readTimeMinutes} min read
        </div>
      </div>
...
function ArticleGridCard({ article, index }: { article: DoctorArticleSummary; index: number }) {
  const emoji = specialtyEmoji(article.category?.name ?? "");
  const thumb = THUMB_COLORS[index % THUMB_COLORS.length];
  const badge = BADGE_COLORS[index % BADGE_COLORS.length];
  return (
    <Link href={`/blog/${article.slug}`} className="art-card-grid" data-cat={article.category?.slug ?? "all"}>
      <div className="art-thumb-grid" style={{ background: thumb }}>
        {emoji}
        <div className="art-badge" style={{ background: badge }}>
          {(article.category?.name ?? "Health").toUpperCase()}
        </div>
      </div>
...
export function DoctorBioPageContent({ doctorId }: { doctorId: string }) {
  ...
  const schedule = useMemo(
    () => asArray<NonNullable<DoctorProfile["weeklySchedule"]>[number]>(doctor?.weeklySchedule),
  ...
  const filteredGridArticles = useMemo(() => {
    ...
    return articles.filter((a) => a.category?.slug === articleFilter);
    ...
    return (
      <div className="doctor-bio-page">
        <div className="page-error">
          ...
          <p>This doctor profile is unavailable or may have been removed.</p>
  ...
  const avgRead = doctor.articleStats?.avgReadTimeMinutes ?? 0;
  const consultations = doctor.patientsTreated ?? doctor.consultationCount ?? 0;
  const fees = doctor.consultationFees ?? {
    video: Number(doctor.consultationFee),
    phone: Math.round(Number(doctor.consultationFee) * 0.75),
    chat: Math.round(Number(doctor.consultationFee) * 0.5),
  };
  const location = [doctor.city, doctor.country].filter(Boolean).join(", ");
  const credLine = [
  ...
  const visibleGrid = filteredGridArticles.slice(0, gridVisible);
        ...
        <div className="hero-inner">
          ...
          <div className="hero-info">
            ...
            <div className="hero-meta">
              ...
              {location && (
                <div className="hero-meta-item">
                  📍 {countryFlag(doctor.country)} {location}
                </div>
              )}
            ...
            <div className="hero-stats">
              ...
              <div className="hero-stat">
                <strong>{formatStatCount(doctor.reviewCount).replace("+", "")}</strong>
                <span>Reviews</span>
              </div>
            </div>
          ...
          <div className="hero-right">
            <button
              type="button"
              className={`follow-big${following ? " following" : ""}`}
              onClick={() => {
                setFollowing((v) => !v);
                showToast(following ? "Unfollowed author" : "Now following author updates");
              }}
            >
              {following ? "✅ Following" : "🔔 Follow Author"}
            </button>
            <p style={{ fontSize: ".72rem", opacity: 0.7, textAlign: "center" }}>
      ...
      <div className="main">
        <div>
          <div className="profile-tabs">
            {(
              [
                ["about", `👤 About`],
                ["articles", `📰 Articles (${articleCount})`],
              ...
              {articlesVisible < articles.length && (
                <div style={{ marginTop: 18, textAlign: "center" }}>
                  <button
                    type="button"
                    className="load-more"
                    onClick={() => setArticlesVisible((n) => n + 6)}
                  >
                    Load More Articles ({articles.length - articlesVisible} remaining)
                  </button>
                </div>
              )}
            </div>
                ...
                <div className="verified-box">
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                    ...
                    <strong style={{ fontSize: ".86rem", color: "var(--green)" }}>
                      PMDC Verification Status: Active
                    </strong>
                  </div>
                  <p style={{ fontSize: ".8rem", color: "#065f46", lineHeight: 1.65 }}>
                    {name}&apos;s medical licence, board certifications, and disciplinary status are
                    verified by DrInsight&apos;s editorial team. No disciplinary proceedings or
                    restrictions exist on this licence as of {verifiedLabel}.
                  </p>
                </div>
              </div>
        ...
        <div className="sidebar">
            ...
            <div className="book-types">
              <div className="book-type">
                ...
                <div className="bt-info">
                  ...
                  <span>45 minutes · HD Video Call</span>
                </div>
                <span className="bt-price">{formatCurrency(fees.video)}</span>
              </div>
              <div className="book-type">
                <span className="bt-icon">📞</span>
                <div className="bt-info">
                  <strong>Phone Consultation</strong>
                  <span>30 minutes · Voice Call</span>
                </div>
                <span className="bt-price">{formatCurrency(fees.phone)}</span>
              </div>
              <div className="book-type">
                <span className="bt-icon">💬</span>
                <div className="bt-info">
                  <strong>Chat Consultation</strong>
                  <span>Async · 24h response</span>
                </div>
                <span className="bt-price">{formatCurrency(fees.chat)}</span>
              </div>
            </div>
          ...
          <div className="sidebar-card">
            <div className="sb-title">📬 Contact {name.replace("Dr. ", "Dr. ")}</div>
            {doctor.user?.email && (
          ...
          {schedule.length > 0 && (
            <div className="sidebar-card">
              <div className="sb-title">🕐 Consultation Availability</div>
              {schedule.map((day) => (
              ...
              {doctor.responseTime && (
                <div style={{ marginTop: 10, fontSize: ".74rem", color: "var(--gray-400)", textAlign: "center" }}>
                  ⏱️ Response time: {doctor.responseTime}
                </div>
              )}
            </div>
          )}
          ...
          <div className="sidebar-card">
            ...
            {doctor.successRate != null && (
              <div className="fact-row">
                <span className="fact-ico">🔬</span>
                <div className="fact-info">
                  <strong>Success Rate</strong>
                  <span>{doctor.successRate}%</span>
                </div>
              </div>
            )}
            {doctor.gender && (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="t-author">
                      {patientName} — {starsDisplay(review.rating)} Verified Patient
                    </div>
                  </div>
                );
              })}
          ...
          <div className="newsletter-sidebar">
            ...
            <p>Get notified when {name} publishes a new article — direct to your inbox.</p>
            <input
              type="email"
            ...
            />
            <button type="button" onClick={onNewsletter} disabled={newsletter.isPending}>
              Follow Author →
            </button>
            <p style={{ fontSize: ".68rem", opacity: 0.6, marginTop: 6 }}>🔒 Unsubscribe anytime.</p>
          </div>
        </div>
      </div>
      ...
      {articles.length > 0 && (
        <div className="articles-section">
          <div className="articles-header">
            ...
            <div className="filter-tabs">
              ...
              {articleCategories.map((cat) => (
                <div
                  key={cat.slug}
                  className={`ftab${articleFilter === cat.slug ? " active" : ""}`}
                  onClick={() => {
                    setArticleFilter(cat.slug);
                    setGridVisible(6);
                  }}
                  role="button"
                  tabIndex={0}
          ...
          <div className="articles-grid">
            {visibleGrid.map((article, index) => (
              <ArticleGridCard key={article.id} article={article} index={index} />
            ))}
          </div>
          {gridVisible < filteredGridArticles.length && (
            <div style={{ textAlign: "center" }}>
              <button type="button" className="load-more" onClick={() => setGridVisible((n) => n + 6)}>
                Load More Articles ({filteredGridArticles.length - gridVisible} remaining)
              </button>
            </div>
          )}
        </div>
        ...
        <div className="eng-inner">
          <div className="eng-left">
            ...
            <button
              ...
              onClick={() => {
                setHelpful("no");
                showToast("We'll improve this profile.");
              }}
            >
